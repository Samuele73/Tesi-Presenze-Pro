...
export class AuthService {
  ...
  get token(){return localStorage.getItem(this.TOKEN_NAME);}
  get email(){return this.userEmail;}
  set email(email: string){this.userEmail = email;}
    ...
    if(token){
      ...
      this.apiService.validateToken(this.token).subscribe({
        next: (resp: any) => {
          ...
          localStorage.setItem("user_status", JSON.stringify({isLogged: true}));
  ...
  signin(userCredentials: userCredentials){
    this.apiService.signInUser(userCredentials);
    return true;
  }
