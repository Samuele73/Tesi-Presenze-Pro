...
export class CustomCalendarPageComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private subscriteToCalendarStateServices(): void {
    this.calendarStateService.calendar
      ...
      .subscribe((calendar: calendar) => {
        ...
        this.hoursWorked = `Ore lavorate: ${this.calculateHoursWorked(
          this.viewDate
        )}`;
        console.log(
          'CALENDAR SUBSCRIPTION, calendar on client',
  ...
  private updatePreCalculatedValues(): void {
    ...
    this.currentMonthYear = `${this.monthNames[this.viewDate.getMonth()]} ${
      this.currentYear
    }`;
  ...
  fromDateToModalTitle(date: Date): string {
    ...
    if (this.dateToTitleCache.size > 50) {
      const firstKey: string | undefined = this.dateToTitleCache
        .keys()
        .next().value;
      if (firstKey) {
